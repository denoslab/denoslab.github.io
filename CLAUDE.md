...
- **Member badges** are achievement discs shown next to a member's name on the team cards and on the profile page. `_data/badges.yml` defines them in display order with a `label`, a disc `color`, a glyph `ink`, and an optional `implies`. A member opts in with `badges: [intern, paper-accepted]` in front matter, and `_includes/badges.html` matches those ids against the data file, so an unknown id is silently skipped and the order on the page is always the data file's order.
  ...
  - Glyphs are `<symbol>` elements in `_includes/badge-sprite.html`, referenced with `<use href="#bdg-<id>">`. The sprite is emitted once per page, from `_includes/team-list.html` for the listings and from `_layouts/profile.html` for a profile. Adding a badge means an entry in the data file and a matching symbol in the sprite.
  - Discs render at 16px with a 10.5px glyph, so **every glyph is one flat silhouette with no small detail**, drawn on a 24 unit grid. Sprout, mortarboard, star, paper plane, and check are deliberately five different shapes, since colour alone does not separate them at that size.
  - The Director and the Postdoc carry no badges. The seeded set covers students, interns, and alumni only.
